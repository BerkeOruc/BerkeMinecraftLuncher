...
[bold cyan]╚═════╝ [/bold cyan]
    ...
    def _install_fabric(self):
        """Fabric kur"""
        self.console.print("\n[bold]Fabric Kurulumu[/bold]")
        self.console.print("[yellow]⚠️ Fabric kurulumu manuel olarak yapılmalıdır[/yellow]")
        self.console.print("[dim]1. https://fabricmc.net/use/installer/ adresine gidin[/dim]")
        self.console.print("[dim]2. Fabric Installer'ı indirin[/dim]")
    ...
    def _show_forge_versions(self):
        ...
        try:
            ...
            if 1 <= choice <= len(forge_versions):
                ...
                self.console.print(f"[yellow]⚠️ Forge sürümleri manuel olarak indirilmelidir.[/yellow]")
                self.console.print(f"[blue]🌐 Forge İndirme: https://files.minecraftforge.net/[/blue]")
                self.console.print(f"[dim]Forge'u indirdikten sonra mods klasörüne yerleştirin.[/dim]")
                input("[dim]Enter...[/dim]")
            else:
                self.console.print("[red]❌ Geçersiz seçim![/red]")
                input("[dim]Enter...[/dim]")
    ...
    def _show_fabric_versions(self):
        ...
        try:
            ...
            if 1 <= choice <= len(fabric_versions):
                ...
                self.console.print(f"[yellow]⚠️ Fabric sürümleri manuel olarak indirilmelidir.[/yellow]")
                self.console.print(f"[blue]🌐 Fabric İndirme: https://fabricmc.net/use/installer/[/blue]")
                self.console.print(f"[dim]Fabric'i indirdikten sonra mods klasörüne yerleştirin.[/dim]")
                input("[dim]Enter...[/dim]")
            else:
                self.console.print("[red]❌ Geçersiz seçim![/red]")
                input("[dim]Enter...[/dim]")
